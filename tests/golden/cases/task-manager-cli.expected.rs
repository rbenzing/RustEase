fn main() {
    if std::env::args().count() as i32 < 2 {
        ...
        let command: String = all_args[1 as usize].clone();
        ...
        if command == String::from("add") {
            if std::env::args().count() as i32 < 3 {
                ...
                let description: String = all_args[2 as usize].clone();
            ...
            if !std::path::Path::new(&filename).exists() {
                ...
                let lines: Vec<String> = content.split(String::from("\n").as_str()).map(|s| s.to_string()).collect::<Vec<String>>();
                let mut count: i32 = 0;
                for line in &lines {
                    if line.starts_with(String::from("[ ] ").as_str()) {
                        count = count + 1;
                        let task: String = line.replace(String::from("[ ] ").as_str(), String::from("").as_str());
                        println!("{}. [ ] {}", count, task);
                    } else if line.starts_with(String::from("[x] ").as_str()) {
                        count = count + 1;
                        let task: String = line.replace(String::from("[x] ").as_str(), String::from("").as_str());
            ...
            if std::env::args().count() as i32 < 3 {
                ...
                let task_num: i32 = all_args[2 as usize].clone().parse::<i32>().unwrap();
                if !std::path::Path::new(&filename).exists() {
